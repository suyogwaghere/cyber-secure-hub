    ...
    const fetchCarbonData = async () => {
      try {
        const response = await fetch(
          `https://api.websitecarbon.com/b?url=${encodeURIComponent(
            initialUrl
          )}`
        );
    ...
    <Card heading={props.title} actionButtons={props.actionButtons}>
      ...
      {carbons?.adjustedBytes > 0 && (
        <>
          ...
          />
          <Row
            lbl="Energy Usage for Load"
            val={`${(carbons.energy * 1000).toPrecision(4)} KWg`}
          />
        </>
      )}
      {carbonData.c && <Row lbl="CO2 Emitted" val={`${carbonData.c} grams`} />}
      {carbonData.p && (
        <Row lbl="Better than average site by" val={`${carbonData.p}%`} />
      )}
      <br />
      <LearnMoreInfo>
        Learn more at{" "}
        <a href="https://www.websitecarbon.com/">websitecarbon.com</a>
      </LearnMoreInfo>
    </Card>
  );
};
