...
const port = process.env.PORT || 3005; // The port to run the server on
...
if (process.env.DISABLE_GUI && process.env.DISABLE_GUI !== "false") {
  app.get("*", async (req, res) => {
    ...
    const htmlContent = placeholderContent.replace(
      "<!-- CONTENT -->",
      "Cyber-Secure-Hub API is up and running!<br />Access the endpoints at " +
...
const printMessage = () => {
  console.log(
    `\x1b[1m\x1b[32m🚀 Cyber-Secure-Hub is up and running at http://localhost:${port} \x1b[0m\n\n`
  );
};
